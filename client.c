...
int add(CLIENT *clnt, int x, int y) {
  operands ops;
  int *result;
  /* junta os parâmetros em um
     struct */
  ops.x = x;
  ops.y = y;
  /* chama a função remota */
  result = add_1(&ops, clnt);
  if (result == NULL) {
    printf("Problemas ao chamar a função remota\n");
    exit(1);
  }
  return (*result);
}

/* função que chama a RPC sub_1 */
...
int main(int argc, char *argv[]) {
  ...
  /* verifica se a referência foi criada */
  if (clnt == (CLIENT *)NULL) {
    clnt_pcreateerror(argv[1]);
    exit(1);
  }
  /* obtém os dois inteiros que serão passados via RPC */
  x = atoi(argv[2]);
  y = atoi(argv[3]);
